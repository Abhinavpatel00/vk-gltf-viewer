...
#define INDEX_SEQ(Is, N, ...)                                                  \
  [&]<std::size_t... Is>(std::index_sequence<Is...>)                           \
      __VA_ARGS__(std::make_index_sequence<N>{})
#define ARRAY_OF(N, ...)                                                       \
  INDEX_SEQ(Is, N, { return std::array{((void)Is, __VA_ARGS__)...}; })
...
vk_gltf_viewer::MainApp::MainApp() {
  ...
  vk::DescriptorPoolCreateInfo descriptorPoolCreateInfo =
      brdfmapComputer.descriptorSetLayout.getPoolSize()
  ...
  const vk::raii::DescriptorPool descriptorPool{gpu.device,
                                                descriptorPoolCreateInfo};

  const auto [brdfmapSet] =
      allocateDescriptorSets(*gpu.device, *descriptorPool,
                             std::tie(brdfmapComputer.descriptorSetLayout));
  gpu.device.updateDescriptorSets(
      brdfmapSet.getWriteOne<0>(
          {{}, *brdfmapImageView, vk::ImageLayout::eGeneral}),
      {});

  const vk::raii::CommandPool computeCommandPool{
      gpu.device, vk::CommandPoolCreateInfo{{}, gpu.queueFamilies.compute}};
  std::variant<vk::CommandPool, vk::raii::CommandPool> graphicsCommandPool =
      *computeCommandPool;
  ...
  const auto [timelineSemaphores, finalWaitValues] =
      vku::executeHierarchicalCommands(
          gpu.device,
          std::forward_as_tuple(
              // Initialize the image based lighting resources by
              // default(white).
              vku::ExecutionInfo{
                  [this](vk::CommandBuffer cb) {
                    ...
                    cb.pipelineBarrier(
                        ...
                        vk::ImageMemoryBarrier{
                            ...
                            vk::QueueFamilyIgnored,
                            imageBasedLightingResources.prefilteredmapImage,
                  ...
                  visit_as<vk::CommandPool>(graphicsCommandPool),
                  gpu.queues.graphicsPresent},
              ...
              vku::ExecutionInfo{
                  [&](vk::CommandBuffer cb) {
                    ...
                    brdfmapComputer.compute(
                        cb, brdfmapSet, vku::toExtent2D(brdfmapImage.extent));
          ...
          std::forward_as_tuple(
              // Acquire BRDF LUT image's queue family ownership from compute to
              // graphicsPresent.
              vku::ExecutionInfo{
                  [&](vk::CommandBuffer cb) {
                    if (gpu.queueFamilies.compute !=
                        gpu.queueFamilies.graphicsPresent) {
                      cb.pipelineBarrier(
                          vk::PipelineStageFlagBits::eTopOfPipe,
                          vk::PipelineStageFlagBits::eBottomOfPipe, {}, {}, {},
  ...
  std::ignore = gpu.device.waitSemaphores(
      {{},
       vku::unsafeProxy(timelineSemaphores | ranges::views::deref |
  ...
  vku::executeSingleCommand(
      *gpu.device, visit_as<vk::CommandPool>(graphicsCommandPool),
      gpu.queues.graphicsPresent,
      [this](vk::CommandBuffer cb) {
      ...
      *fence);
  std::ignore =
      gpu.device.waitForFences(*fence, true, ~0ULL); // TODO: failure handling
...
#elif __linux__
      "/usr/share/fonts/noto/NotoSansMono-Medium.ttf", // use more general linux font from font dir 
#else
#error "Type your own font file in here!"
  ...
  const vk::Format colorAttachmentFormat = gpu.supportSwapchainMutableFormat
  ...
  ImGui_ImplVulkan_InitInfo initInfo{
      ...
      // ImGui requires ImGui_ImplVulkan_InitInfo::{MinImageCount,ImageCount} ≥
      // 2 (I don't know why...).
...
vk_gltf_viewer::MainApp::~MainApp() {
  ...
  if (skyboxResources) {
    ImGui_ImplVulkan_RemoveTexture(
        skyboxResources->imGuiEqmapTextureDescriptorSet);
  }
...
void vk_gltf_viewer::MainApp::run() {
  ...
  std::array frames =
      ARRAY_OF(FRAMES_IN_FLIGHT, vulkan::Frame{gpu, sharedData});

  gpu.device.updateDescriptorSets(
      {
          sharedData.imageBasedLightingDescriptorSet.getWriteOne<0>(
              {imageBasedLightingResources.cubemapSphericalHarmonicsBuffer, 0,
               vk::WholeSize}),
          sharedData.imageBasedLightingDescriptorSet.getWriteOne<1>(
              {{},
               *imageBasedLightingResources.prefilteredmapImageView,
               vk::ImageLayout::eShaderReadOnlyOptimal}),
          sharedData.imageBasedLightingDescriptorSet.getWriteOne<2>(
              {{}, *brdfmapImageView, vk::ImageLayout::eShaderReadOnlyOptimal}),
      },
      {});

  // Booleans that indicates frame at the corresponding index should handle
  // swapchain resizing.
  std::array<bool, FRAMES_IN_FLIGHT> shouldHandleSwapchainResize{};

  std::vector<control::Task> tasks;
  for (std::uint64_t frameIndex = 0; !glfwWindowShouldClose(window);
    ...
    static vk::Rect2D passthruRect{};
    {
      control::ImGuiTaskCollector imguiTaskCollector{
          tasks,
          ImVec2{static_cast<float>(swapchainExtent.width),
                 static_cast<float>(swapchainExtent.height)},
          passthruRect,
      };

      imguiTaskCollector.menuBar(appState.getRecentGltfPaths(),
                                 appState.getRecentSkyboxPaths());
      if (auto &gltfAsset = appState.gltfAsset) {
        imguiTaskCollector.assetInspector(gltfAsset->asset, gltf->directory);
        imguiTaskCollector.materialEditor(
            gltfAsset->asset, gltfAsset->assetInspectorMaterialIndex,
            assetTextureDescriptorSets);
        imguiTaskCollector.sceneHierarchy(
            gltfAsset->asset, gltfAsset->getSceneIndex(),
            gltfAsset->nodeVisibilities, gltfAsset->hoveringNodeIndex,
            gltfAsset->selectedNodeIndices);
        imguiTaskCollector.nodeInspector(gltfAsset->asset,
                                         gltfAsset->selectedNodeIndices);
      }
      if (const auto &iblInfo = appState.imageBasedLightingProperties) {
        imguiTaskCollector.imageBasedLighting(
            *iblInfo, skyboxResources->imGuiEqmapTextureDescriptorSet);
      }
      imguiTaskCollector.background(appState.canSelectSkyboxBackground,
                                    appState.background);
      imguiTaskCollector.inputControl(
          appState.camera, appState.automaticNearFarPlaneAdjustment,
          appState.useFrustumCulling, appState.hoveringNodeOutline,
          appState.selectedNodeOutline);
      if (appState.gltfAsset &&
          appState.gltfAsset->selectedNodeIndices.size() == 1) {
        const std::size_t selectedNodeIndex =
            *appState.gltfAsset->selectedNodeIndices.begin();
        imguiTaskCollector.imguizmo(
            appState.camera,
            gltf->sceneHierarchy.nodeWorldTransforms[selectedNodeIndex],
      ...
      visit(
          multilambda{
              ...
              [&](const control::task::LoadGltf &task) {
                // TODO: I'm aware that there are more good solutions than
                // waitIdle, but I don't have much time for it
                ...
                try {
                  ...
                  std::println(std::cerr,
                               "The glTF file cannot be processed because of "
                               "an error: {}",
                               to_string(error));
                  return;
                } catch (fastgltf::Error error) {
                  // If error is due to missing or unknown required extension,
                  // show a message and return.
                  if (ranges::one_of(
                          error, fastgltf::Error::MissingExtensions,
                          fastgltf::Error::UnknownRequiredExtension)) {
                    std::println(std::cerr,
                                 "The glTF file requires an extension that is "
                ...
                imageInfos.emplace_back(
                    *sharedData.singleTexelSampler,
                    *gpuFallbackTexture.imageView,
                    vk::ImageLayout::eShaderReadOnlyOptimal);
                imageInfos.append_range(
                    gltf->asset.textures |
                    std::views::transform([this](const fastgltf::Texture
                                                     &texture) {
                      return vk::DescriptorImageInfo{
                          to_optional(texture.samplerIndex)
                              .transform([this](std::size_t samplerIndex) {
                                return *gltf->assetGpuTextures
                                            .samplers[samplerIndex];
                              })
                              .value_or(*gpuFallbackTexture.sampler),
                          *gltf->assetGpuTextures.imageViews.at(
                              gltf::AssetGpuTextures::getPreferredImageIndex(
                                  texture)),
                          vk::ImageLayout::eShaderReadOnlyOptimal,
                      };
                    }));
                gpu.device.updateDescriptorSets(
                    {
                        sharedData.assetDescriptorSet.getWriteOne<0>(
                            {gltf->assetGpuBuffers.primitiveBuffer, 0,
                             vk::WholeSize}),
                        sharedData.assetDescriptorSet.getWriteOne<1>(
                            {gltf->assetGpuBuffers.materialBuffer, 0,
                             vk::WholeSize}),
                        sharedData.assetDescriptorSet.getWrite<2>(imageInfos),
                        sharedData.sceneDescriptorSet.getWriteOne<0>(
                            {gltf->sceneGpuBuffers.nodeBuffer, 0,
                             vk::WholeSize}),
                    },
                    {});

                // TODO: due to the ImGui's gamma correction issue, base
                // color/emissive texture is rendered darker than it should be.
                assetTextureDescriptorSets =
                    gltf->asset.textures |
                ...
                const float distance =
                    radius / std::sin(appState.camera.fov / 2.f);
                appState.camera.position =
                    glm::make_vec3(center.data()) -
                    glm::dvec3{distance * normalize(appState.camera.direction)};
                ...
                gpu.device.updateDescriptorSets(
                    {
                        ...
                        sharedData.imageBasedLightingDescriptorSet
                            .getWriteOne<1>(
                                {{},
                                 *imageBasedLightingResources
                                      .prefilteredmapImageView,
                                 vk::ImageLayout::eShaderReadOnlyOptimal}),
                        sharedData.skyboxDescriptorSet.getWriteOne<0>(
                            {{},
              ...
              [&](control::task::ChangeScene task) {
                ...
                gpu.device.updateDescriptorSets(
                    sharedData.sceneDescriptorSet.getWriteOne<0>(
                        {gltf->sceneGpuBuffers.nodeBuffer, 0, vk::WholeSize}),
                    {});
                          ...
                          [&](std::span<std::optional<bool>> visibilities) {
                            ...
                            tristate::propagateTopDown(
                                [&](auto i) -> decltype(auto) {
                                  return gltf->asset.nodes[i].children;
                                },
                                task.nodeIndex, visibilities);
                            tristate::propagateBottomUp(
                                [&](auto i) {
                                  return gltf->sceneHierarchy
              ...
              [this](const control::task::ChangeNodeLocalTransform &task) {
                fastgltf::math::fmat4x4 nodeWorldTransform = visit(
                    fastgltf::visitor{
                ...
                if (auto parentNodeIndex =
                        gltf->sceneHierarchy.getParentNodeIndex(
                            task.nodeIndex)) {
                  nodeWorldTransform =
                      gltf->sceneHierarchy
                          .nodeWorldTransforms[*parentNodeIndex] *
                      nodeWorldTransform;
                }

                // Update the current and its descendant nodes' world transforms
                // in sceneHierarchy.
                gltf->sceneHierarchy.updateDescendantNodeTransformsFrom(
                    task.nodeIndex, nodeWorldTransform);

                // Passing sceneHierarchy into sceneGpuBuffers to update GPU
                // mesh node transform buffer.
                gltf->sceneGpuBuffers.updateMeshNodeTransformsFrom(
                    task.nodeIndex, gltf->sceneHierarchy,
                    gltf->assetExternalBuffers);

                // Scene enclosing sphere would be changed. Adjust the camera's
                // near/far plane if necessary.
                if (appState.automaticNearFarPlaneAdjustment) {
                  const auto &[center, radius] = gltf->sceneMiniball =
                      gltf::algorithm::getMiniball(
                          gltf->asset, gltf->scene,
                          [this](std::size_t nodeIndex,
                                 std::size_t instanceIndex) {
                            return cast<double>(
                                gltf->sceneGpuBuffers.getMeshNodeWorldTransform(
                                    nodeIndex, instanceIndex));
                          });
                  appState.camera.tightenNearFar(glm::make_vec3(center.data()),
                                                 radius);
                }
              },
              [this](control::task::ChangeSelectedNodeWorldTransform) {
                ...
                // own local transformation matrix. When the node has no parent
                // node, its global transformation matrix is identical to its
                // local transformation matrix.
                //
                // (node world transform matrix) = (parent node world transform
                // matrix) * (node local transform matrix).
                // => (node local transform matrix) = (parent node world
                // transform matrix)^-1 * (node world transform matrix).

                // TODO: replace this function with fastgltf provided if it
                // exported.
                static constexpr auto affineInverse =
                    []<typename T>(
                        const fastgltf::math::mat<T, 4, 4> &m) noexcept {
                      ...
                      return fastgltf::math::mat<T, 4, 4>(
                          fastgltf::math::vec<T, 4>(inv.col(0).x(),
                                                    inv.col(0).y(),
                                                    inv.col(0).z(), 0.f),
                          fastgltf::math::vec<T, 4>(inv.col(1).x(),
                                                    inv.col(1).y(),
                                                    inv.col(1).z(), 0.f),
                          fastgltf::math::vec<T, 4>(inv.col(2).x(),
                                                    inv.col(2).y(),
                                                    inv.col(2).z(), 0.f),
                              ...
                              const fastgltf::math::fmat4x4 transformMatrix =
                                  affineInverse(
                                      gltf->sceneHierarchy.nodeWorldTransforms
                                          [*parentNodeIndex]) *
                                  selectedNodeWorldTransform;
                              decomposeTransformMatrix(transformMatrix,
                                                       trs.scale, trs.rotation,
                                                       trs.translation);
                            } else {
                              decomposeTransformMatrix(
                                  selectedNodeWorldTransform, trs.scale,
                ...
                // in sceneHierarchy.
                gltf->sceneHierarchy.updateDescendantNodeTransformsFrom(
                    selectedNodeIndex, selectedNodeWorldTransform);

                // Passing sceneHierarchy into sceneGpuBuffers to update GPU
                // mesh node transform buffer.
                gltf->sceneGpuBuffers.updateMeshNodeTransformsFrom(
                    selectedNodeIndex, gltf->sceneHierarchy,
                    gltf->assetExternalBuffers);
    ...
    const vulkan::Frame::UpdateResult updateResult = frame.update({
        ...
        .camera = {appState.camera.getViewMatrix(),
                   appState.camera.getProjectionMatrix()},
        .frustum = value_if(appState.useFrustumCulling,
                            [this]() { return appState.camera.getFrustum(); }),
        .cursorPosFromPassthruRectTopLeft =
            appState.hoveringMousePosition.and_then(
                [&](const glm::vec2 &position) -> std::optional<vk::Offset2D> {
                  // If cursor is outside the framebuffer, cursor position is
                  // undefined.
                  const glm::vec2 framebufferSize = window.getFramebufferSize();
                  const glm::vec2 framebufferCursorPosition =
                      position * framebufferSize / glm::vec2{window.getSize()};
                  if (framebufferCursorPosition.x >= framebufferSize.x ||
                      framebufferCursorPosition.y >= framebufferSize.y)
                    return std::nullopt;

                  const vk::Offset2D offset{
                      static_cast<std::int32_t>(framebufferCursorPosition.x) -
                          passthruRect.offset.x,
                      static_cast<std::int32_t>(framebufferCursorPosition.y) -
                          passthruRect.offset.y};
                  return value_if(0 <= offset.x &&
                                      offset.x < passthruRect.extent.width &&
                                      0 <= offset.y &&
                                      offset.y < passthruRect.extent.height,
                                  offset);
                }),
        .gltf = gltf.transform([&](Gltf &gltf) {
          assert(appState.gltfAsset &&
                 "Synchronization error: gltfAsset is not set in AppState.");
          return vulkan::Frame::ExecutionTask::Gltf{
              ...
              .hoveringNode = transform(
                  [&](std::uint16_t index, const AppState::Outline &outline) {
                  ...
                  appState.gltfAsset->hoveringNodeIndex,
                  appState.hoveringNodeOutline.to_optional()),
              .selectedNodes = value_if(
                  !appState.gltfAsset->selectedNodeIndices.empty() &&
                      appState.selectedNodeOutline.has_value(),
                  [&]() {
        ...
        .handleSwapchainResize = std::exchange(
            shouldHandleSwapchainResize[frameIndex % frames.size()], false),
    });
    ...
    try {
      ...
      // Note: ignoring vk::Result is okay because it would be handled by the
      // outer try-catch block.
      const std::uint32_t swapchainImageIndex =
          (*gpu.device)
              .acquireNextImageKHR(*swapchain, ~0ULL,
                                   frame.getSwapchainImageAcquireSemaphore())
              .value;
      ...
      while (!glfwWindowShouldClose(window) &&
             (swapchainExtent = getSwapchainExtent()) == vk::Extent2D{}) {
      ...
      const vk::raii::Fence fence{gpu.device, vk::FenceCreateInfo{}};
      vku::executeSingleCommand(
          *gpu.device, *graphicsCommandPool, gpu.queues.graphicsPresent,
          [&](vk::CommandBuffer cb) {
            recordSwapchainImageLayoutTransitionCommands(cb);
          },
          *fence);
      std::ignore = gpu.device.waitForFences(*fence, true,
                                             ~0ULL); // TODO: failure handling
...
vk_gltf_viewer::MainApp::Gltf::Gltf(fastgltf::Parser &parser,
    ...
    : dataBuffer{get_checked(fastgltf::GltfDataBuffer::FromPath(path))},
      ...
      asset{get_checked(parser.loadGltf(dataBuffer, directory))}, gpu{gpu},
      ...
      sceneMiniball{gltf::algorithm::getMiniball(
          asset, scene,
          [this](std::size_t nodeIndex, std::size_t instanceIndex) {
            return cast<double>(sceneGpuBuffers.getMeshNodeWorldTransform(
                nodeIndex, instanceIndex));
          })} {}
...
void vk_gltf_viewer::MainApp::Gltf::setScene(std::size_t sceneIndex) {
  ...
  sceneGpuBuffers = {asset, scene, sceneHierarchy, gpu, assetExternalBuffers};
  sceneMiniball = gltf::algorithm::getMiniball(
      asset, scene, [this](std::size_t nodeIndex, std::size_t instanceIndex) {
        return cast<double>(sceneGpuBuffers.getMeshNodeWorldTransform(
            nodeIndex, instanceIndex));
      });
}
...
auto vk_gltf_viewer::MainApp::createInstance() const -> vk::raii::Instance {
  ...
  extensions.append_range(
      std::views::counted(glfwExtensions, glfwExtensionCount));
...
vk_gltf_viewer::MainApp::createSwapchain(vk::SwapchainKHR oldSwapchain) const {
  const vk::SurfaceKHR surface = window.getSurface();
  const vk::SurfaceCapabilitiesKHR surfaceCapabilities =
      gpu.physicalDevice.getSurfaceCapabilitiesKHR(surface);
  const auto viewFormats = {vk::Format::eB8G8R8A8Srgb,
                            vk::Format::eB8G8R8A8Unorm};
...
auto vk_gltf_viewer::MainApp::createDefaultImageBasedLightingResources() const
    -> ImageBasedLightingResources {
  vku::MappedBuffer sphericalHarmonicsBuffer{
      gpu.allocator, std::from_range,
      std::array<glm::vec3, 9>{
          glm::vec3{1.f},
      },
      vk::BufferUsageFlagBits::eUniformBuffer};
  vku::AllocatedImage prefilteredmapImage{
      gpu.allocator, vk::ImageCreateInfo{
...
auto vk_gltf_viewer::MainApp::createBrdfmapImage() const
    -> decltype(brdfmapImage) {
...
auto vk_gltf_viewer::MainApp::createImGuiDescriptorPool()
    -> decltype(imGuiDescriptorPool) {
  return {
      gpu.device,
      vk::DescriptorPoolCreateInfo{
          ...
          1       /* Default ImGui rendering */
              ...
              +   /*static_cast<std::uint32_t>(gltf->asset.textures.size())*/
              512 /* material textures */, // TODO: need proper texture count.
          vku::unsafeProxy(vk::DescriptorPoolSize{
              vk::DescriptorType::eCombinedImageSampler,
              1       /* Default ImGui rendering */
                  + 1 /* reducedEqmapImage texture */
                  + /*static_cast<std::uint32_t>(gltf->asset.textures.size())*/
                  512 /* material textures */ // TODO: need proper texture
                                              // count.
          }),
      }};
}
...
auto vk_gltf_viewer::MainApp::processEqmapChange(
    const std::filesystem::path &eqmapPath) -> void {
  const auto [eqmapImageExtent, eqmapStagingBuffer] = [&]() {
    if (auto extension = eqmapPath.extension(); extension == ".hdr") {
      int width, height;
      std::unique_ptr<float[]>
          data; // It should be freed after copying to the staging buffer,
                // therefore declared as unique_ptr.
      data.reset(
          stbi_loadf(PATH_C_STR(eqmapPath), &width, &height, nullptr, 4));
      if (!data) {
      ...
      vku::MappedBuffer buffer{gpu.allocator,
                               vk::BufferCreateInfo{
                                   {},
                                   blockSize(vk::Format::eR32G32B32A32Sfloat) *
                                       eqmapExtent.width * eqmapExtent.height,
                                   vk::BufferUsageFlagBits::eTransferSrc,
                               }};
      ...
      frameBuffer.insert("R", Imf::Slice{Imf::FLOAT,
                                         reinterpret_cast<char *>(&data[0].x),
                                         sizeof(glm::vec4), rowBytes});
      frameBuffer.insert("G", Imf::Slice{Imf::FLOAT,
                                         reinterpret_cast<char *>(&data[0].y),
                                         sizeof(glm::vec4), rowBytes});
      frameBuffer.insert("B", Imf::Slice{Imf::FLOAT,
                                         reinterpret_cast<char *>(&data[0].z),
                                         sizeof(glm::vec4), rowBytes});
    ...
    throw std::runtime_error{
        "Unknown file format: only HDR and EXR are supported."};
  }();

  std::uint32_t eqmapImageMipLevels = 0;
  for (std::uint32_t mipWidth = eqmapImageExtent.width; mipWidth > 512;
       mipWidth >>= 1, ++eqmapImageMipLevels)
    ;
  ...
  const vk::Extent3D reducedEqmapImageExtent =
      eqmapImage.mipExtent(eqmapImage.mipLevels - 1);
  vku::AllocatedImage reducedEqmapImage{
      gpu.allocator, vk::ImageCreateInfo{
                         ...
                         vku::Image::maxMipLevels(reducedEqmapImageExtent),
                         1,
                         vk::SampleCountFlagBits::e1,
                         vk::ImageTiling::eOptimal,
                         vk::ImageUsageFlagBits::eTransferDst |
  ...
  constexpr vulkan::MipmappedCubemapGenerator::Config
      mippedCubemapGeneratorConfig{
          .cubemapSize = 1024,
          .cubemapUsage = vk::ImageUsageFlagBits::eSampled,
      };
  vulkan::MipmappedCubemapGenerator mippedCubemapGenerator{
      gpu, mippedCubemapGeneratorConfig};
  const vulkan::MipmappedCubemapGenerator::Pipelines
      mippedCubemapGeneratorPipelines{
          vulkan::pipeline::CubemapComputer{gpu.device},
          vulkan::pipeline::SubgroupMipmapComputer{
              gpu, vku::Image::maxMipLevels(
                       mippedCubemapGeneratorConfig.cubemapSize)},
      };

  // Generate IBL resources.
  constexpr vulkan::ImageBasedLightingResourceGenerator::Config
      iblGeneratorConfig{
          .prefilteredmapImageUsage = vk::ImageUsageFlagBits::eSampled,
          .sphericalHarmonicsBufferUsage =
              vk::BufferUsageFlagBits::eUniformBuffer,
      };
  vulkan::ImageBasedLightingResourceGenerator iblGenerator{gpu,
                                                           iblGeneratorConfig};
  const vulkan::ImageBasedLightingResourceGenerator::Pipelines
      iblGeneratorPipelines{
          vulkan::pipeline::PrefilteredmapComputer{
              gpu,
              {vku::Image::maxMipLevels(iblGeneratorConfig.prefilteredmapSize),
               1024}},
          vulkan::pipeline::SphericalHarmonicsComputer{gpu.device},
          vulkan::pipeline::SphericalHarmonicCoefficientsSumComputer{
              gpu.device},
  ...
  const vulkan::CubemapToneMappingRenderer cubemapToneMappingRenderer{
      gpu.device, cubemapToneMappingRenderPass};

  vku::AllocatedImage toneMappedCubemapImage{
      gpu.allocator, vk::ImageCreateInfo{
                         vk::ImageCreateFlagBits::eCubeCompatible,
                         vk::ImageType::e2D,
                         vk::Format::eB8G8R8A8Srgb,
                         ...
                         1,
                         mippedCubemapGenerator.cubemapImage.arrayLayers,
                         vk::SampleCountFlagBits::e1,
                         vk::ImageTiling::eOptimal,
                         vk::ImageUsageFlagBits::eColorAttachment |
                             vk::ImageUsageFlagBits::eSampled,
  ...
  const vk::raii::ImageView cubemapImageArrayView{
      gpu.device,
      mippedCubemapGenerator.cubemapImage.getViewCreateInfo(
          {vk::ImageAspectFlagBits::eColor, 0, 1, 0, 6},
          vk::ImageViewType::e2DArray),
  };
  const vk::raii::ImageView toneMappedCubemapImageArrayView{
      gpu.device,
      toneMappedCubemapImage.getViewCreateInfo(vk::ImageViewType::e2DArray)};

  const vk::raii::DescriptorPool cubemapToneMappingDescriptorPool{
      gpu.device, getPoolSizes(cubemapToneMappingRenderer.descriptorSetLayout)
                      .getDescriptorPoolCreateInfo()};
  const auto [cubemapToneMappingDescriptorSet] = allocateDescriptorSets(
      *gpu.device, *cubemapToneMappingDescriptorPool,
      std::tie(cubemapToneMappingRenderer.descriptorSetLayout));
  gpu.device.updateDescriptorSets(
      cubemapToneMappingDescriptorSet.getWriteOne<0>(
          {{},
           *cubemapImageArrayView,
           vk::ImageLayout::eShaderReadOnlyOptimal}),
      {});

  const vk::raii::Framebuffer cubemapToneMappingFramebuffer{
      gpu.device, vk::FramebufferCreateInfo{
                      {},
                      cubemapToneMappingRenderPass,
                      *toneMappedCubemapImageArrayView,
                      toneMappedCubemapImage.extent.width,
                      toneMappedCubemapImage.extent.height,
                      1,
                  }};

  const vk::raii::CommandPool transferCommandPool{
      gpu.device, vk::CommandPoolCreateInfo{{}, gpu.queueFamilies.transfer}};
  std::variant<vk::CommandPool, vk::raii::CommandPool> computeCommandPool =
      *transferCommandPool;
  std::variant<vk::CommandPool, vk::raii::CommandPool> graphicsCommandPool =
      *transferCommandPool;
  ...
  const auto [timelineSemaphores, finalWaitValues] =
      executeHierarchicalCommands(
          ...
          std::forward_as_tuple(
              // Generate eqmapImage mipmaps and blit its last mip level image
              // to reducedEqmapImage.
              vku::ExecutionInfo{
                  [&](vk::CommandBuffer cb) {
                    if (gpu.queueFamilies.transfer !=
                        gpu.queueFamilies.graphicsPresent) {
                      cb.pipelineBarrier(
                          vk::PipelineStageFlagBits::eTopOfPipe,
                          vk::PipelineStageFlagBits::eTransfer, {}, {}, {},
                    ...
                    // Blit from eqmapImage[level=-1] to
                    // reducedEqmapImage[level=0], whose extents are the same.
                    cb.blitImage(
                        eqmapImage, vk::ImageLayout::eTransferSrcOptimal,
                        reducedEqmapImage, vk::ImageLayout::eTransferDstOptimal,
                        vk::ImageBlit{
                            {vk::ImageAspectFlagBits::eColor,
                             eqmapImage.mipLevels - 1, 0, 1},
                            {vk::Offset3D{},
                             vku::toOffset3D(eqmapImage.mipExtent(
                                 eqmapImage.mipLevels - 1))},
                            {vk::ImageAspectFlagBits::eColor, 0, 0, 1},
                            {vk::Offset3D{},
                             vku::toOffset3D(reducedEqmapImage.extent)},
                        },
                        vk::Filter::eLinear);

                    // eqmapImage[level=0] will be used as sampled image (other
                    // mip levels will not be used).
                    cb.pipelineBarrier(
                        vk::PipelineStageFlagBits::eTransfer,
                        vk::PipelineStageFlagBits::eBottomOfPipe, {}, {}, {},
                        vk::ImageMemoryBarrier{
          ...
          std::forward_as_tuple(
              ...
              vku::ExecutionInfo{
                  [&](vk::CommandBuffer cb) {
                    ...
                    // Generate reducedEqmapImage mipmaps.
                    vulkan::recordMipmapGenerationCommand(cb,
                                                          reducedEqmapImage);
                  ...
                  visit_as<vk::CommandPool>(graphicsCommandPool),
                  gpu.queues.graphicsPresent /*, 4*/},
              // Generate cubemap with mipmaps from eqmapImage, and generate IBL
              // resources from the cubemap.
              vku::ExecutionInfo{
                  [&](vk::CommandBuffer cb) {
                    if (gpu.queueFamilies.graphicsPresent !=
                        gpu.queueFamilies.compute) {
                      // Do queue family ownership transfer from graphicsPresent
                      // to compute, if required.
                      cb.pipelineBarrier(
                          vk::PipelineStageFlagBits::eTopOfPipe,
                          vk::PipelineStageFlagBits::eComputeShader, {}, {}, {},
                          vk::ImageMemoryBarrier{
                    ...
                    mippedCubemapGenerator.recordCommands(
                        cb, mippedCubemapGeneratorPipelines, eqmapImage);
                    ...
                    cb.pipelineBarrier(
                        ...
                        vk::ImageMemoryBarrier{
                            ...
                            vk::QueueFamilyIgnored,
                            vk::QueueFamilyIgnored,
                            mippedCubemapGenerator.cubemapImage,
                            vku::fullSubresourceRange(),
                        });

                    iblGenerator.recordCommands(
                        cb, iblGeneratorPipelines,
                        mippedCubemapGenerator.cubemapImage);
                    ...
                    cb.pipelineBarrier(
                        ...
                        vk::PipelineStageFlagBits::eBottomOfPipe, {}, {}, {},
                        {
                            ...
                            vk::ImageMemoryBarrier{
                                ...
                                gpu.queueFamilies.compute,
                                gpu.queueFamilies.graphicsPresent,
                                iblGenerator.prefilteredmapImage,
                                vku::fullSubresourceRange(),
                            },
                        });
                  },
                  visit_as<vk::CommandPool>(computeCommandPool),
                  gpu.queues.compute}),
          std::forward_as_tuple(
              // Acquire resources' queue family ownership from compute to
              // graphicsPresent (if necessary), and create tone mapped cubemap
              // image (=toneMappedCubemapImage) from high-precision image
              // (=mippedCubemapGenerator.cubemapImage).
              vku::ExecutionInfo{
                  [&](vk::CommandBuffer cb) {
                    if (gpu.queueFamilies.compute !=
                    ...
                    cb.beginRenderPass(
                        {
                            ...
                            *cubemapToneMappingFramebuffer,
                            vk::Rect2D{
                                {0, 0},
                                vku::toExtent2D(toneMappedCubemapImage.extent)},
                    ...
                    cb.bindPipeline(vk::PipelineBindPoint::eGraphics,
                                    *cubemapToneMappingRenderer.pipeline);
                    cb.bindDescriptorSets(
                        vk::PipelineBindPoint::eGraphics,
                        *cubemapToneMappingRenderer.pipelineLayout, 0,
                        cubemapToneMappingDescriptorSet, {});
                    cb.setViewport(
                        0, vku::unsafeProxy(vku::toViewport(vku::toExtent2D(
                               toneMappedCubemapImage.extent))));
                    cb.setScissor(
                        0,
                        vku::unsafeProxy(vk::Rect2D{
                            {0, 0},
                            vku::toExtent2D(toneMappedCubemapImage.extent)}));
                    cb.draw(3, 1, 0, 0);
  ...
  std::ranges::copy(
      ...
      appState.imageBasedLightingProperties->diffuseIrradiance
          .sphericalHarmonicCoefficients.begin());

  if (skyboxResources) {
    // Since a descriptor set allocated using ImGui_ImplVulkan_AddTexture cannot
    // be updated, it has to be freed and re-allocated (which done in below).
    (*gpu.device)
        .freeDescriptorSets(*imGuiDescriptorPool,
                            skyboxResources->imGuiEqmapTextureDescriptorSet);
  }

  // Emplace the results into skyboxResources and imageBasedLightingResources.
  vk::raii::ImageView reducedEqmapImageView{
      gpu.device, reducedEqmapImage.getViewCreateInfo()};
  const vk::DescriptorSet imGuiEqmapImageDescriptorSet =
      ImGui_ImplVulkan_AddTexture(*reducedEqmapSampler, *reducedEqmapImageView,
                                  VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
  vk::raii::ImageView toneMappedCubemapImageView{
      gpu.device,
      toneMappedCubemapImage.getViewCreateInfo(vk::ImageViewType::eCube)};
  skyboxResources.emplace(
      std::move(reducedEqmapImage), std::move(reducedEqmapImageView),
      std::move(toneMappedCubemapImage), std::move(toneMappedCubemapImageView),
      imGuiEqmapImageDescriptorSet);

  vk::raii::ImageView prefilteredmapImageView{
      gpu.device, iblGenerator.prefilteredmapImage.getViewCreateInfo(
...
void vk_gltf_viewer::MainApp::recordSwapchainImageLayoutTransitionCommands(
    vk::CommandBuffer cb) const {
  cb.pipelineBarrier(
      vk::PipelineStageFlagBits::eTopOfPipe,
      vk::PipelineStageFlagBits::eBottomOfPipe, {}, {}, {},
      swapchainImages | std::views::transform([](vk::Image swapchainImage) {
