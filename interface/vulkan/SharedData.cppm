...
namespace vk_gltf_viewer::vulkan {
    ...
    // Ensure the flag for freeing descriptor sets is included
    vk::DescriptorPoolCreateFlags flags = vk::DescriptorPoolCreateFlagBits::eFreeDescriptorSet | vk::DescriptorPoolCreateFlagBits::eUpdateAfterBind;
    
    // Get the pool sizes using your existing method
    auto poolCreateInfo = getPoolSizes(assetDescriptorSetLayout).getDescriptorPoolCreateInfo(flags);

    // Return the created descriptor pool
    return { gpu.device, poolCreateInfo };
}

[[nodiscard]] auto createDescriptorPool() const -> vk::raii::DescriptorPool {
    // Combine the flags required for the descriptor pool
    vk::DescriptorPoolCreateFlags flags = vk::DescriptorPoolCreateFlagBits::eFreeDescriptorSet;

    // Get the pool sizes and descriptor pool creation info
    auto poolCreateInfo = getPoolSizes(imageBasedLightingDescriptorSetLayout,
                                       sceneDescriptorSetLayout, 
                                       skyboxDescriptorSetLayout)
                              .getDescriptorPoolCreateInfo(flags);
